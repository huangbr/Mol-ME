...
class JointLoss(nn.Module):
    ...
    def forward(self, output, target, vec0=None, vec1=None):
        ...
        elif vec0 is None or vec1 is None:
            loss = self.loss(output, target)
        else:
...
def bulid_loss(cfg, weights=None):
    """
    Build loss criterion for multi-task learning.
    Always returns a list of JointLoss instances, one per task,
    ensuring consistent indexing regardless of task type or weights availability.
    """
    num_tasks = len(cfg.DATA.TASK_NAME)
    criterion_list = []

    for i in range(num_tasks):
        if cfg.DATA.TASK_TYPE == 'classification':
            weight_i = weights[i] if weights is not None else None
            if weight_i is not None:
                weight_i = torch.Tensor(weight_i).to(
                    torch.device("cuda" if torch.cuda.is_available() else "cpu")
                )
                loss = nn.CrossEntropyLoss(weight=weight_i) if not cfg.LOSS.FL_LOSS else FocalLoss(alpha=1/weight_i[0])
            else:
        ...
        joint_loss = JointLoss(
            loss=loss,
            cl_loss=cl_loss,
            alpha=cfg.LOSS.ALPHA if hasattr(cfg.LOSS, 'ALPHA') else 0.5
        )
        criterion_list.append(joint_loss)
