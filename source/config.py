...
# Metric, choice from ['acc','auc', 'prc', 'rmse', 'mae']
_C.DATA.METRIC = 'auc'
# anti-noise rate for hiv dataset, only for noise split type
_C.DATA.RATE = None
# Augmentation Factor for data enhancement
_C.DATA.AUG_FACTOR = 10
...
# GNN:初始化GNN节点
_C.MODEL.GNN = CN()
# Hidden size of MolME model
_C.MODEL.GNN.HID_DIM = 64
# Output size of MolME model, override by dataset.py 474
_C.MODEL.GNN.OUT_DIM = None
# Number of layers
_C.MODEL.GNN.DEPTH = 3
# Number of heads
_C.MODEL.GNN.SLICES = 2
# Dropout
_C.MODEL.GNN.DROPOUT = 0.2
# Feature attention
_C.MODEL.GNN.F_ATT = True
# reduction value
_C.MODEL.GNN.R = 4
# Whether to use BRICS information, if set to False, the option LOSS.CL_LOSS is set to False
_C.MODEL.GNN.BRICS = True

# SEQ
_C.MODEL.SEQ = CN()
_C.MODEL.SEQ.OUT_DIM = 768

# ATTENTION
_C.MODEL.ATT = CN()
_C.MODEL.ATT.NUM_HEADS = 4
_C.MODEL.ATT.DROPOUT = 0.1
...
def _update_config_from_file(config, cfg_file):
    config.defrost() # 解除配置节点的冻结状态，允许对其进行修改
