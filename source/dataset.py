...
class MolDataset(InMemoryDataset):
    ...
    def process(self):
        ...
        for i, smi in enumerate(tqdm(smilesList)):
            ...
            if data is not None:
                label = target[i]
                label[np.isnan(label)] = 2 # 标签缺失则标记为2
                data.y = torch.LongTensor([label])
                # 预处理数据：加入smiles_vec
                data.smiles_vec = smiles_vec[i]
                if self.task_type == 'regression':
...
def build_loader(cfg, logger):
    ...
    # train_dataloader = GeometricDataLoader(train_dataset, batch_size=cfg.DATA.BATCH_SIZE, shuffle=True)
    # valid_dataloader = GeometricDataLoader(valid_dataset, batch_size=cfg.DATA.BATCH_SIZE)
    # test_dataloader = GeometricDataLoader(test_dataset, batch_size=cfg.DATA.BATCH_SIZE)
