...
# -----------------------------------------------------------------------------
# Bond-type markers for molecular framework encoding
# Used in molecular property calculations and scaffold decomposition
# -----------------------------------------------------------------------------
# Standard bond-type encoding for framework atom classification
# SP2 hybridization bond marker: represents double-bond or aromatic connectivity
FRAMEWORK_BOND_SP2 = 8203
# SP3 hybridization bond marker: represents single-bond connectivity
FRAMEWORK_BOND_SP3 = 8204
...
def load_best_result(cfg, model, logger):
    ...
    # 若从未存过最优权重（例如训练早期一直非有限、验证集从未改善），
    # 回退到当前内存中的模型，避免因文件缺失直接崩溃。
    if not os.path.exists(best_ckpt_path):
        logger.info(f'Warning: no best_ckpt found at {best_ckpt_path}, use current in-memory model')
        return model, 0
...
def parse_args():
    parser = argparse.ArgumentParser(description="codes for MolME")
...
def print_result(cfg, args, logger, train, num_trials):
    ...
    if cfg.DATA.TASK_TYPE == 'classification':
        # Classification: 仅报告基于验证集选择的最佳集成学习模型测试分数
        for trial in range(num_trials):
            test_scores, best_test_score, best_val_score, best_model_name, train_time = train(args, cfg, logger)
            test_score = round(np.nanmean(test_scores)*100, 1)
            best_test_score = round(best_test_score*100, 1)
            best_val_score = round(best_val_score*100, 1)

            test_score_list.append(test_score)
            best_test_score_list.append(best_test_score)
            best_val_score_list.append(best_val_score)
            best_model_name_list.append(best_model_name.upper())
        ...
        for trail in range(num_trials):
            ...
            logger.info('Training time {}'.format(train_time_list[trail]))
            logger.info('Test score {:.1f}'.format(best_test_score_list[trail]))
        # 计算均值和标准差
        mean_accuracy = np.mean(best_test_score_list)
        std_accuracy = np.std(best_test_score_list)
        logger.info('-' * 40)
        ...
        # Regression: 仅报告基于验证集选择的最佳集成学习模型测试分数
        metric_upper = cfg.DATA.METRIC.upper()
        for trial in range(num_trials):
            test_scores, best_test_score, best_val_score, best_model_name, train_time = train(args, cfg, logger)
            test_score = round(np.nanmean(test_scores), 6)
            best_test_score = round(best_test_score, 6)
        ...
        for trail in range(num_trials):
            ...
            logger.info('Trial {}'.format(trail + 1))
            logger.info('Training time {}'.format(train_time_list[trail]))
            logger.info('Test score {:.6f}'.format(best_test_score_list[trail]))
        # 计算均值和标准差
        mean_accuracy = np.mean(best_test_score_list)
        std_accuracy = np.std(best_test_score_list)
        logger.info('-' * 40)
